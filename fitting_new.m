...
default_parameters.pha_1_NAA=2.1;
default_parameters.pha_0_Cr=2.8;
default_parameters.pha_1_Cr=3.1;
default_parameters.pha_0_Cho=3.3;
default_parameters.pha_1_Cho=3.4;
...
%         re_bl =fnval(sp1,ppm);
        re_bl = ssa(real(data_value.FID_FD(:,i)),50); % estimate the baseline for real part
%         sp2 = csaps(ppm,imag(data_value.FID_FD(:,i)),30);
%         im_bl = fnval(sp2,ppm);
        im_bl = ssa(imag(data_value.FID_FD(:,i)),50); % estimate the baseline for imaginary part
...
%                 re_bl =fnval(sp1,ppm);
                re_bl = ssa(real(data_value.FID_FD(:,i)-signal_fitted(:,count-1)),50); % estimate the baseline for real part
%                 sp2 = csaps(ppm,imag(data_value.FID_FD(:,i)-signal_fitted(:,count-1)),30);
%                 im_bl =fnval(sp2,ppm);
                im_bl = ssa(imag(data_value.FID_FD(:,i)-signal_fitted(:,count-1)),50); % estimate the baseline for imaginary part
                ...
                if (std(abs(baseline(:,count)-baseline(:,count-1)))<=1e-5 || count==2)
                    break;
                end
                count=count+1;
        end
%         [ fit_sig,amp,Ind_sig,freq,Z_sig,FWHM] = HSVDPK_new(data_value.FID_TD_b(:,i),details_para.fres,details_para.Fs,fit_para.order(cur_vox));
    ...
    %     [fit_sig,Ind_sig,Z_sig] = model_estimate(fit_param11,details_para.t,model_select);
        ...
        data_value.fit_TD(:,i)=data_value.fit_TD(:,i) + baseline_t(:,count-1);%baseline_t(:,count-1);%data_value.fit_TD(:,i) + ifft(ifftshift(data_value.FID_FD_base(:,i)));
        data_value.fit_FD(:,i)=fftshift(fft(data_value.fit_TD(:,i)));
...
x_max = 850;
y_max = 600;
col_name = {'Area','SNR','NAA/Cr','Cho/Cr'};
